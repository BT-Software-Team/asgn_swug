...
> **Note column pending** — the source specification also documents operational notes per flag (e.g., which flags propagate to other calibrator-dependent analyses); not yet ported here.

## Analysis Identifier Reference

> Pending. This will decode the `Analysis` / `Measurement under Consideration` identifiers that appear in `quality_control.csv` (e.g., `CFTR_POLYTTG`, `[GeneName]_CN`, `[Paralog1_Paralog2]_sum`).
...
> **Additional columns pending** — the source specification lists several more (Unanalyzed Super/Partially Spanning RC, Partial Span Fwd/Rev Found RC, and 4×/3× median comparisons) not yet ported here.
